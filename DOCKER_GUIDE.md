...
# Filter for cost and performance metrics
docker compose logs -f | grep "✓"
```

**Optimized Log Output:**
- ✅ **No HTTP access logs** - Cleaner, focused output
- ✅ **Cost per request** - Real-time cost tracking
- ✅ **Performance metrics** - Time and token usage
- ✅ **Request IDs** - Easy tracking and debugging

**Example Log Entry:**
```
INFO: [abc-123] → Processing email rewrite...
INFO: [abc-123] ✓ Email rewritten | Time: 3.45s | Tokens: 428 | Cost: $0.0257
```
...
docker exec email-rewriter curl http://localhost:8000/health
```

### Debugging
```bash
# Enter container shell
docker exec -it email-rewriter /bin/bash

# Check container resource usage
docker stats email-rewriter

# Inspect container details
docker inspect email-rewriter

# View container processes
docker top email-rewriter
```
...
docker rm email-rewriter

# Remove specific image
docker rmi p1-email-rewriter
```
...
docker images | grep email-rewriter
...
docker inspect email-rewriter | grep -A 5 Mounts
...
docker exec email-rewriter env | grep OPENAI
...
docker exec email-rewriter env | grep OPENAI_API_KEY

# Verify Python packages
docker exec email-rewriter pip list
```
...
### Using curl
```bash
# Upload PDF resume and TXT email context
...
2. Find `/rewrite-upload` endpoint
3. Click "Try it out"
...
# One-time stats
docker stats --no-stream email-rewriter
```

### Check API Response Times
```bash
# Time a request
time curl -X POST "http://localhost:8000/rewrite" \
     -H "Content-Type: application/json" \
     -d '{"email text":"test","email context":"test"}'
```
